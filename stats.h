...
private:
	...
	* @param lr is (x,y) of the lower right corner of the rectangle */
	...
	unsigned char getChannelPixel(vector<vector<long>>* vect, PNG & im, unsigned int x, unsigned int y);
	vector<vector<long>>* getVector(char channel, int sq);
	long getSumsHelper(char channel, int sq, pair<int,int> ul, pair<int,int> lr);
